...
function activeFile() {
	...
	if (
		!editor ||
		!isFileUri(editor.document.uri) ||
		isExcludedFile(editor.document.uri.fsPath)
	)
		return undefined;
...
function openFiles() {
	...
	const addUri = (uri) => {
		if (!isFileUri(uri) || isExcludedFile(uri.fsPath) || seen.has(uri.fsPath))
			return;
		seen.add(uri.fsPath);
...
function updateStatus(payload) {
	...
	statusItem.tooltip = contextFile
		? `Pi context: ${contextFile}`
		: "Pi context";
	statusItem.show();
}
